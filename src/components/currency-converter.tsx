...
export function CurrencyConverter() {
  ...
  const [fromCurrency, setFromCurrency] = useState<"INR" | "USD" | "AUD">("INR");
  ...
  const calculateConversions = useCallback((value: string, from: "INR" | "USD" | "AUD") => {
    ...
    if (from === 'INR') {
        setInrValue(value);
        setUsdValue((numericValue * CONVERSION_RATES.INR_TO_USD).toFixed(2));
        setAudValue((numericValue * CONVERSION_RATES.INR_TO_AUD).toFixed(2));
    } else if (from === 'USD') {
        const inr = numericValue * CONVERSION_RATES.USD_TO_INR;
        setInrValue(inr.toFixed(2));
        setUsdValue(value);
        setAudValue((inr * CONVERSION_RATES.INR_TO_AUD).toFixed(2));
    } else { // from AUD
        const inr = numericValue * CONVERSION_RATES.AUD_TO_INR;
        setInrValue(inr.toFixed(2));
        setUsdValue((inr * CONVERSION_RATES.INR_TO_USD).toFixed(2));
        setAudValue(value);
    }
  }, []);
  ...
  }, [amount, fromCurrency, calculateConversions]);
